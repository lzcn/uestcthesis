...
\title{The \cls{uestcthesis} package}
\author{Zhi Lu\thanks{Please report any issues at \url{https://github.com/lzcn/uestcthesis/issues}}}
\date{2022-06-27 \quad v0.0.1}
\maketitle
\begin{abstract}
  本模板基于中科大的\href{https://github.com/ustctug/uestcthesis}{论文模板}项目修改，旨在提供一个易于后续维护的非官方版本。
\end{abstract}
\tableofcontents

\section{简介}

本模板 \cls{uestcthesis} 是中国科学技术大学本科生和研究生学位论文的 \LaTeX{}
...
下载地址：
\begin{itemize}
  \item 主要地址：\url{https://github.com/ustctug/uestcthesis/releases}
  \item 学校镜像：\url{https://git.lug.ustc.edu.cn/ustctug/uestcthesis}
  \item 研究生院网站（版本较旧）：
    \url{https://gradschool.ustc.edu.cn/column/65}
\end{itemize}

用户在使用 \pkg{uestcthesis} 模板前，应先阅读学校的《撰写手册》等规范。
如果在使用的过程中遇到问题，可以阅读
\href{https://github.com/ustctug/uestcthesis/wiki}{常见问题}，
或者在 \href{https://github.com/ustctug/uestcthesis/issues}{GitHub Issues}
中反馈。
...
\begin{table}[htb]
  ...
  \begin{tabular}{lll}
    ...
    类别     & 文件                      & 说明                         \\
    \midrule
    模板文件 & \file{uestcthesis.cls}     & 模板文件                     \\
             & \file{uestcthesis-*.bst}   & \BibTeX{} 参考文献表格式     \\
             & \file{uestcthesis-*.bbx}   & BibLaTeX 参考文献表格式      \\
             & \file{uestcthesis-*.cbx}   & BibLaTeX 参考文献表格式      \\
             & \file{figures/uestc-*.pdf} & 校名和校徽图片               \\
    \midrule
    使用说明 & \file{uestcthesis-doc.tex} & 模板使用说明的源代码         \\
             & \file{uestcthesis-doc.pdf} & （你正在阅读的）模板使用说明 \\
    \midrule
    示例文档 & \file{main.tex}           & 主文档                       \\
             & \file{uestcsetup.tex}      & 配置文件                     \\
             & \file{chapters/*.tex}     & 示例文档的各个章节           \\
             & \file{figures/}           & 放置图片的目录               \\
             & \file{bib/uestc.bib}       & \BibTeX{} 示例数据库         \\
    \midrule
...
\begin{shell}
  make
\end{shell}
编译说明文档 \file{uestcthesis-doc.pdf}：
\begin{shell}
  make doc
\end{shell}
...
\begin{latex}
  \documentclass[degree-type=professional]{uestcthesis}
\end{latex}

\DescribeOption{language}
论文全文的主要语言。可选：\opt{chinese}（默认），\opt{english}。
\begin{latex}
  \documentclass[language=english]{uestcthesis}
\end{latex}
...
\end{itemize}
\begin{latex}
  \documentclass[output=electronic]{uestcthesis}
\end{latex}
...
\end{itemize}
\begin{latex}
  \documentclass[section-style=arabic]{uestcthesis}
\end{latex}
...
\begin{latex}
  \documentclass[fontset=windows]{uestcthesis}
\end{latex}
...
封面由 \cs{maketitle} 命令生成，其中的各项信息使用 \cs{uestcsetup} 命令以
key-value 的方式填写，如：
\begin{latex}
  \uestcsetup{
    title  = {论文中文题目},
...
有几点说明：
\begin{itemize}
  \item \cs{uestcsetup} 使用 \pkg{kvsetkeys} 机制，配置项之间不能有空行，否则会报错。
...
有的院系额外要求声明页有“评审专家签名”，可以在模板中使用 \cs{uestcsetup} 命令设置
\opt{reviewer = true}。
...
对于特殊的章节，\cls{uestcthesis} 还提供了相应的环境：
\begin{itemize}
...
\DescribeOption{keywords*}
摘要的关键词应使用 \cs{uestcsetup} 的接口进行设置，在录入时使用西文逗号隔开，
比如：
\begin{latex}
  \begin{abstract}
    这里是摘要。
...
\begin{latex}
  ...
  \bibliographystyle{uestcthesis-numeric}
\end{latex}
或者
\begin{latex}
  % 著者-出版年制
...
\begin{latex}
  \usepackage[bibstyle=uestcthesis-numeric,
              citestyle=uestcthesis-inline]{biblatex}
\end{latex}
